      ...
      content: (
        <div className="pf-vizstack">
          ...
          <p className="pf-note">{ci.ciRatio < 0.85
            ...
            {isReal && <> {es ? 'Esperado en dato real: mag/grav/lab son físicamente correlacionadas, así que el test se dispara. No es una trampa plantada, es la realidad geofísica; por eso la regresión logística, que no supone independencia condicional, es la ruta a seguir.' : 'Expected on real data: mag/grav/lab are physically correlated, so the test fires. This is not a planted trap, it is the geophysical reality; hence logistic regression, which does not assume conditional independence, is the route to follow.'}</>}</p>
        </div>
      ),
    },
    {
      ...
      content: (
        <div className="pf-vizstack">
          <div className="pf-plot-t">{es
            ? `El mismo modelo (WofE) bajo CV aleatorio vs CV espacial-por-bloques: ${analysis.cv.k} folds, bloques de ${analysis.cv.blockCells}x${analysis.cv.blockCells} celdas, umbrales, pesos y prior ajustados solo con las celdas de los folds de entrenamiento, cada celda del mapa puntuada una vez mientras está retenida, un único AUC sobre los scores agrupados. ${analysis.cv.inflationGap > 0 ? 'El gap es la inflación por autocorrelación espacial (el AUC aleatorio miente).' : 'Aquí el AUC con CV aleatorio no supera al espacial, así que este caso no muestra inflación.'}`
            : `The same model (WofE) under random CV vs spatial-block CV: ${analysis.cv.k} folds, ${analysis.cv.blockCells}x${analysis.cv.blockCells}-cell blocks, thresholds, weights and prior fitted on the training folds' cells only, every map cell scored once while held out, one AUC over the pooled scores. ${analysis.cv.inflationGap > 0 ? 'The gap is the inflation from spatial autocorrelation (the random AUC lies).' : 'Here the random-CV AUC is not above the spatial one, so this case shows no inflation.'}`}</div>
          <div className="pf-kpis">
          ...
          <p className="pf-note">{es
            ? 'El número reportado por defecto es siempre el espacial. El aleatorio se muestra solo para exponer la inflación.'
            : 'The default reported number is always the spatial one. The random one is shown only to expose the inflation.'}
            {isReal && <> {es ? 'En el belt MVT real las ocurrencias están muy agrupadas (distrito Tri-State): el AUC con CV espacial queda muy por debajo del AUC con CV aleatorio y del AUC de ajuste, así que la geofísica regional sola tiene poca habilidad de transferencia espacial.' : 'On the real MVT belt the occurrences are strongly clustered (the Tri-State district): the spatial-CV AUC is well below the random-CV AUC and the fitting AUC, so regional geophysics alone has limited spatial-transfer skill.'}</>}</p>
        </div>
      ),
    },
    {
        ...
        <div className="pf-vizstack">
          ...
          {!learnedField ? (
            ...
            <>
              ...
              <p className="pf-note">{es
                ? 'El WofE de caja blanca es la autoridad interpretable. El CV aleatorio se muestra solo para exponer la inflación (aleatorio menos espacial). Solo se comparan valores medidos con el mismo protocolo.'
                : 'The white-box WofE is the interpretable authority. Random CV is shown only to expose the inflation (random minus spatial). Only values measured under the same protocol are compared.'}
                {isReal && clf?.nocv && <> {es
        ...
        <div className="pf-vizstack">
          ...
          {!isReal ? (
            ...
            <>
              ...
              <p className="pf-note">{es
                ? `Resultado: sobre el belt MVT agrupado, PU-Conformal (AUC CV bloques contiguos ${num(puConformal?.benchmark?.find((b) => b.model === 'pu_conformal')?.auc)}) no supera al WofE en las mismas particiones (${num(puConformal?.benchmark?.find((b) => b.model === 'wofe')?.auc)}) en ranking; el null de distancia-a-depósito ya alcanza ${num(puConformal?.negative_controls?.distance_to_deposit_null?.distance_to_deposit_auc)}. La cobertura conforme se cumple, pero solo marcando ~${pct(confLevel?.set_size_frac ?? 0, 0)} del belt: un conjunto casi vacío, que muestra que la geofísica regional no localiza MVT bajo transferencia espacial. El avance es la incertidumbre calibrada y corregida por sesgo, no un AUC mayor.`
                : `Result: over the clustered MVT belt, PU-Conformal (contiguous-fold CV AUC ${num(puConformal?.benchmark?.find((b) => b.model === 'pu_conformal')?.auc)}) does not beat WofE on the same folds (${num(puConformal?.benchmark?.find((b) => b.model === 'wofe')?.auc)}) in ranking; the distance-to-deposit null alone reaches ${num(puConformal?.negative_controls?.distance_to_deposit_null?.distance_to_deposit_auc)}. Conformal coverage holds, but only by flagging ~${pct(confLevel?.set_size_frac ?? 0, 0)} of the belt: a near-vacuous set, which shows that regional geophysics does not localize MVT under spatial transfer. The advance is calibrated, bias-corrected uncertainty, not a higher AUC.`}</p>
            </>
          )}
        </div>
      ),
