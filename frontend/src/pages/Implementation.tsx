...
export default function Implementation() {
  ...
  return (
    <article className="page-body prose">
      ...
      <p className="lede">{es
        ? 'Un motor TypeScript sin dependencias se ejecuta en vivo en el navegador y en el precálculo offline; dos contratos de datos en Python + un pipeline numpy-light + un carril torch→ONNX. Cómputo pesado offline → artefacto compacto → cliente delgado.'
      ...
      <Callout variant="note" title={es ? 'El precálculo en dos lenguajes' : 'The two-language bake'}>
        {es
          ? 'science/bake_cases.mjs importa el mismo motor TS (vía tsx) y ejecuta analyze.ts sobre cada caso → case-results.json. El pipeline Python (numpy-light) reestructura eso en los traces+manifests por caso (Contrato 2). Así los números en vivo y offline son idénticos por construcción.'
      ...
      <p>{es
        ? 'El entrenamiento usa el estimador de riesgo no negativo (nnPU) tratando todas las celdas no-depósito como no etiquetadas, particiones espaciales contiguas idénticas para los seis modelos, y split-conformal de clase positiva calibrado sobre bloques separados. pi se barre como parámetro de sensibilidad (SCAR es dudoso por sesgo de exploración). El veredicto de ranking se lee directamente de los IC bootstrap y los controles.'
        : 'Training uses the non-negative risk estimator (nnPU) treating all non-deposit cells as unlabeled, identical contiguous spatial folds for all six models, and positive-class split-conformal calibrated on separated blocks. pi is swept as a sensitivity parameter (SCAR is doubtful under exploration bias). The ranking verdict is read directly off the bootstrap CIs and the controls.'}</p>

      <h2>{es ? 'Verificación' : 'Verification'}</h2>
      <p>{es
        ? 'node:test (17 pruebas del motor: WofE de forma cerrada, equivalencia WofE↔LR, el omnibus ≈1 vs T>N(D) en la trampa CI, las curvas de captura, el gap de inflación, la cross-validation completamente fuera de fold), ruff, pytest (los 2 contratos, el head-to-head aprendido, los datos del manuscrito), python data-pipeline/run.py all (11 casos), check_artifacts (Contrato 2), re-run byte-idéntico, npm run build.'
        : 'node:test (17 engine tests: closed-form WofE, the WofE↔LR equivalence, the omnibus ≈1 vs T>N(D) on the CI trap, the capture curves, the inflation gap, the fully out-of-fold cross-validation), ruff, pytest (the 2 contracts, the learned head-to-head, the manuscript data), python data-pipeline/run.py all (11 cases), check_artifacts (Contract 2), byte-identical re-run, npm run build.'}</p>
    </article>
  );
}
