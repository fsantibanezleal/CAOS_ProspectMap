          ...
          content: (
            <div className="pf-doc-sec">
              ...
              <Callout variant="note" title={es ? 'El umbral de contraste máximo' : 'The maximizing-contrast threshold'}>
                {es ? 'Una capa continua se binariza en el umbral ' : 'A continuous layer is binarized at the threshold '}<InlineMath tex="t^{*}=\arg\max_t C(t)" />{es ? ' (sujeto a un piso de studC). Arrastrar el umbral fuera de ' : ' (subject to a studC floor). Dragging the threshold off '}<InlineMath tex="t^{*}" />{es ? ' degrada el contraste — la lección didáctica central.' : ' degrades the contrast — the central didactic lesson.'} <Cite id="bonhamcarter1994" paren />
              </Callout>
            </div>
          ),
        },
        ...
        },
        {
          ...
          content: (
            <div className="pf-doc-sec">
              ...
              <p>{es ? 'Sobre patrones binarios condicionalmente independientes, ' : 'On conditionally-independent binary patterns, '}<InlineMath tex="\beta_j\approx C_j" />{es ? ' (la equivalencia WofE↔LR). Con violación de CI, los ' : ' (the WofE↔LR equivalence). Under CI violation the '}<InlineMath tex="\beta_j" />{es ? ' se encogen y no sobre-estiman. Se ajusta por IRLS con ridge (el conjunto positivo es pequeño y desbalanceado).' : ' shrink and do not over-estimate. Fit by IRLS with ridge (the positive set is small and imbalanced).'}</p>
            </div>
          ),
        },
        {
