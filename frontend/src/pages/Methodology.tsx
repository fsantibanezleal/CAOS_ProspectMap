...
export default function Methodology() {
  ...
  return (
    <article className="page-body prose">
      ...
      <p className="lede">{es
        ? 'Weights of Evidence, el test de independencia condicional, la regresión logística (la generalización sin CI) y la validación por cross-validation espacial.'
        : 'Weights of Evidence, the conditional-independence test, logistic regression (the CI-free generalization) and validation by spatial cross-validation.'}</p>
        ...
        },
        {
          ...
          content: (
            <div className="pf-doc-sec">
              ...
              <p>{es ? 'La cross-validation de WofE es completamente fuera de fold: en cada fold, el umbral de máximo contraste de cada capa, sus pesos W+ y W- y el prior se ajustan solo con las celdas y los depósitos de los folds de entrenamiento, y el fold retenido solo se puntúa. Ninguna de sus celdas, con o sin depósito, entra en el ajuste, así que su etiqueta no puede mover el umbral que la puntúa. Cada celda del mapa se puntúa una vez y los scores retenidos se agrupan en un único AUC.' : "The WofE cross-validation is fully out of fold: in each fold, every layer's maximizing-contrast threshold, its W+ and W- and the prior are fitted on the training folds' cells and deposits only, and the held-out fold is only scored. None of its cells, with or without a deposit, enters the fit, so a held-out label cannot move the threshold that scores it. Every map cell is scored once and the held-out scores are pooled into one AUC."}</p>
          ...
          content: (
            <div className="pf-doc-sec">
              ...
              <Callout variant="honest" title={es ? 'Exchangeabilidad rota' : 'Exchangeability broken'}>
                {es ? 'La garantía conforme supone exchangeabilidad, que la autocorrelación espacial rompe. Bajo bloqueo espacial la garantía es marginal sobre bloques y se degrada con el shift de distribución bloque-a-bloque. Sobre MVT muy agrupado los conjuntos pueden ser casi vacíos (marcar casi todo el belt): esa banda ancha es el hallazgo ' : 'The conformal guarantee assumes exchangeability, which spatial autocorrelation breaks. Under spatial blocking the guarantee is marginal over blocks and degrades under block-to-block distribution shift. On strongly clustered MVT the sets can be near-vacuous (flagging almost the whole belt): that wide band is the finding '}<Cite id="roberts2017" paren />{es ? ', no un defecto.' : ', not a defect.'}
              </Callout>
            </div>
          ),
        },
